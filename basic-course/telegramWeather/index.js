...
bot.onText(/\/start/, function onPhotoText(msg) {
  bot.sendMessage(
    msg.chat.id,
    `Hey, my name is Dick. I'd like to help and show you the weather in certain cities.`
  );
});
...
bot.onText(/\/forecast/, (msg) => {
  ...
  bot.sendMessage(msg.chat.id, "Select the city", options);
  ...
  bot.onText(cityRegex, (cityMsg) => {
    bot.sendMessage(msg.chat.id, "Select the interval", {
      reply_markup: JSON.stringify({
        keyboard: [[{ text: "every 3 hours" }], [{ text: "every 6 hours" }]],
      }),
    });
